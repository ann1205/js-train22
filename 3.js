...
class Store {
  ...
  createProduct(name) {
    ...
    this.sendNotify(`Новий продукт "${name}" в магазині ${this.name}!`);
    this.sendEmail(`Новий продукт "${name}" в магазині ${this.name}!`);
  }
   ...
   */
  sendEmail(product) {
    this.customers.forEach((customer) => customer.send(product));
  }
